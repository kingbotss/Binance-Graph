...
class InstagramBot {
    ...
    constructor() {
        ...
        this.command=require('readline-sync');
        //this.fp=require('puppeteer-afp')
    }
async SetBrowser(path){
...
//puppeteer.use(StealthPlugin());
       // let cpath=this.command.question("Paste Chrome Location\n")
        this.browser = await puppeteer.connect({
            ...
            args: ['--no-sandbox'],
           "browserURL":this.connection
           //'C:/Program Files/Google/Chrome/Application/chrome.exe'
          
        });
        this.page = await this.browser.newPage();
       //this.page=this.fp(this.page);
    ...
    async SetEmail(){
        await this.page.goto("https://emailfake.com/",{waitUntil:"networkidle2"});
        ...
        let uniqnum=Math.floor(Math.random() * 789)+123;
        this.uni=uniqnum;
        this.uname=randomName;
        //await this.page.$eval("input#userName",el(val)=>el.value=val,randomName);
        await this.page.evaluate((number)=>{
          document.querySelector("input#userName").value=number;
        },randomName);
        await this.page.waitFor(2500);
        let ens=await this.page.$$eval(".e7m.tt-suggestions div",divs=>divs.length);
        var randomNum=Math.floor(Math.random() * ens);
        await this.page.waitFor(2500);
        await this.page.evaluate((number)=>{
          document.querySelectorAll(".e7m.tt-suggestions div")[number].querySelector("p").click();
        },randomNum);
        try{
        let email1=await this.page.$eval("#email_ch_text",el=>el.innerText);
        console.log(email1);
        this.email1=email1;
        }catch(e){}
        }
        async RegisterAccount(){
          let agents=require('user-agents')
          const userAgent = new agents(/^Safari|iPhone/);
        await this.page.setUserAgent(userAgent.toString());
        console.log(userAgent.toString());
        this.page.setViewport({height:Number(userAgent.data.viewportHeight),width:Number(userAgent.data.viewportWidth),deviceScaleFactor:1})
        await this.page.goto(this.config.base_url,{waitUntil:"networkidle2"});
        await this.page.waitFor(2000);
        
        await this.CallSignup();
        ...
        async CallSignup(){
          ...
          if(await this.page.$("a[href*='emailsignup']")!=null){
            await this.page.waitFor(1500);
            console.log("email signup appeared.")
            await this.WaitToClick("a[href*='emailsignup']");
            await this.page.waitForNavigation();
          
          }
          if (await this.page.$(".pbNvD[role=dialog]") !== null){
            //onetap 
             await this.page.waitFor(Math.floor(Math.random() *500)+1500);
            await this.page.click("button.bIiDR");
            //wait for page redirect
         
        }
        
          if(await this.page.$("a[href*='/signup/phone']")!=null){
            await this.page.waitFor(1500);
        console.log("signup appeared.")
            await this.WaitToClick("a[href*='/signup/phone']");
           // await this.page.waitForNavigation();
          
          }
          if(await this.page.$("[role=switch]")!=null){
            await this.page.waitFor(1500);
            //console.log("email super");
           //let list= await this.page.$$eval("[role=switch]",els=>els.map((el)=>el.innerText));
            await this.page.click("[role=switch]:nth-child(2)");
            await this.page.waitFor(2500);
           await this.WaitToClick("input[type=email]");
           await this.page.keyboard.type(this.email1,{delay:100});
           await this.page.waitFor(2500);
           await this.WaitToClick("button.y3zKF");
          }
        
        
          if(await this.page.$("button.y3zKF")!=null){
            await this.page.waitFor(1500);
            let text=await this.page.$eval("button.y3zKF",el=>el.innerText);
          ...
          if(await this.page.$("input[name=emailConfirmationCode]")!=null){
            await this.page.waitFor(1500);
            //console.log("email super");
           //let list= await this.page.$$eval("[role=switch]",els=>els.map((el)=>el.innerText));
           // await this.page.click("[role=switch]:nth-child(2)");
           await this.WaitToClick("input[name=emailConfirmationCode]");
         
         let coder= await this.GetCode();
         console.log("verify code:"+coder);
         await this.page.waitFor(3500);
           await this.page.keyboard.type(String(coder),{delay:100});
           await this.WaitToClick("button.cB_4K");
          }
          if(await this.page.$("form input[name=emailOrPhone]")!=null){
            await this.page.waitFor(1500);
          await this.WaitToClick("form input[name=emailOrPhone]");
          await this.page.keyboard.type("myuniqusername");
          //form input[name=emailOrPhone]
          //form input[name=fullName]
          //form input[name=username]
          //form input[name=password]
          
          console.log("wait till fill fields");
          
          }
        if(pageurl.includes('accounts/signup/name')){
          await this.WaitToClick("input[name=fullName]");
          await this.page.waitFor(5500);
           await this.page.keyboard.type(this.uname,{delay:100});
           await this.WaitToClick("input[name=password]");
           let byear=Math.floor(Math.random() *12)+1985;
        let pass=this.uname.toLowerCase()+this.uni+byear;
        await this.page.waitFor(3500);
        console.log(pass);
           await this.page.keyboard.type(String(pass),{delay:100});
           await this.page.waitFor(2500);
           await this.WaitToClick("button.cB_4K");
        }
        if(pageurl.includes('accounts/signup/birthday')){
          try{
          let bday=Math.floor(Math.random() *28)+1;
          let bmon=Math.floor(Math.random() *11)+1;
          let byear=Math.floor(Math.random() *12)+1985;
        
          await this.page.evaluate((selector,valr) => {
            var upload = document.querySelector(selector);
            upload.value=valr;
            upload.dispatchEvent(new Event('change', { bubbles: true }))
        }, 'select[title*=Day]',bday);
        await this.page.waitFor(Math.floor(Math.random() *2500)+3500);
        await this.page.evaluate((selector,valr) => {
          var upload = document.querySelector(selector);
          upload.value=valr;
          upload.dispatchEvent(new Event('change', { bubbles: true }))
        }, 'select[title*=Month]',bmon);
        await this.page.waitFor(Math.floor(Math.random() *2500)+3500);
        await this.page.evaluate((selector,valr) => {
          var upload = document.querySelector(selector);
          upload.value=valr;
          upload.dispatchEvent(new Event('change', { bubbles: true }))
        }, 'select[title*=Year]',byear);
        await this.page.waitFor(Math.floor(Math.random() *3500)+3500);
           await this.WaitToClick("button.cB_4K");
          }catch(e){}
        
        }
        if(pageurl.includes('accounts/signup/username')){
        
           await this.WaitToClick("button.cB_4K");
        }
        
        if(pageurl.includes('/challenge/')){
        
          console.log("Challeng Was Shown");
        }
          await this.CallSignup();
        }
        ...
        async GetCode(){
         ...
         this.page2=page2;}else{
          page2=this.page2;
         }
         
        await page2.goto("https://emailfake.com",{waitUntil:"networkidle2"});
        await page2.waitFor(2000);
        try{
          let otp=await page2.evaluate(()=>{
           return document.querySelector(".list-group-item-info [class*=subj]").innerText.match(/[0-9]{6}/gi);
            
          });
         // console.log(otp);
          if(Number(otp.toString())>1000){
        
            await page2.evaluate(()=>{
              document.querySelector(".ma-2.btn-danger[onclick]").scrollIntoView();
              return document.querySelector(".ma-2.btn-danger[onclick]").click(); 
             });
             await page2.waitFor(2000);
            console.log(otp[0]);
          await page2.close();
          this.page2=undefined;
         this.otp=Number(otp[0]);
...
return countr.length;
    ...
    async visitInstagram() { 
       ...
       await this.page.goto("https://browserleaks.com/canvas");
        await this.page.waitFor(2500);
        ...
        let uname=this.command.question("Enter Username\n")
        await this.page.keyboard.type(uname,{delay:100});
        await this.page.click(this.config.selectors.password_field);
        let upass=this.command.question("Enter Password\n",{hideEchoBack: true})
        await this.page.keyboard.type(upass,{delay:100});
